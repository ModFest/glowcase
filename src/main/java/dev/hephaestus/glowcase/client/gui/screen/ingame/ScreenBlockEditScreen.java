...
public class ScreenBlockEditScreen extends BlockEditorScreen<ScreenBlockEntity> {
	...
	public ScreenBlockEditScreen(ScreenBlockEntity blockEntity) {
		super(blockEntity);
	}
	...
	protected void init() {
		...
		this.widthEntryWidget.setValue("" + this.blockEntity.width);
		...
		if (this.blockEntity.pitch == 0.0f) {
			this.pitchEntryWidget.setValue("");
			this.pitchEntryWidget.setHint(TextUtils.placeholder("gui.glowcase.pitch"));
		} else {
			this.pitchEntryWidget.setValue(String.valueOf(this.blockEntity.pitch));
		}
		this.pitchEntryWidget.setResponder(string -> {
			if (string.isEmpty()) {
				blockEntity.pitch = 0f;
			} else if (Floats.tryParse(string) instanceof Float parsed) {
				blockEntity.pitch = parsed;
			}
		});
		...
		this.offsetXField.setValue("" + this.blockEntity.preciseX);
		this.offsetXField.setResponder(string -> {
			if (Floats.tryParse(string) instanceof Float parsed) {
				blockEntity.preciseX = parsed;
			}
		});

		this.offsetYField = new EditBox(this.minecraft.font, leftX + fieldWidth + gap, fieldY + 15, fieldWidth, 20, Component.empty());
		this.offsetYField.setValue("" + this.blockEntity.preciseY);
		this.offsetYField.setResponder(string -> {
			if (Floats.tryParse(string) instanceof Float parsed) {
				blockEntity.preciseY = parsed;
			}
		});

		this.offsetZField = new EditBox(this.minecraft.font, leftX + 2 * (fieldWidth + gap), fieldY + 15, fieldWidth, 20, Component.empty());
		this.offsetZField.setValue("" + this.blockEntity.preciseZ);
		this.offsetZField.setResponder(string -> {
			if (Floats.tryParse(string) instanceof Float parsed) {
				blockEntity.preciseZ = parsed;
			}
		});

		this.zOffsetToggle = Button.builder(Component.translatable(switch (this.blockEntity.zOffset) {
			case NEGATIVE -> "gui.glowcase.back";
			case NULL -> "gui.glowcase.center";
			case POSITIVE -> "gui.glowcase.front";
		}), action -> {
			switch (blockEntity.zOffset) {
				case POSITIVE -> blockEntity.zOffset = ScreenBlockEntity.Offset.NULL;
				case NULL -> blockEntity.zOffset = ScreenBlockEntity.Offset.NEGATIVE;
				case NEGATIVE -> blockEntity.zOffset = ScreenBlockEntity.Offset.POSITIVE;
			}
			this.zOffsetToggle.setMessage(Component.translatable(switch (this.blockEntity.zOffset) {
				case NEGATIVE -> "gui.glowcase.back";
				case NULL -> "gui.glowcase.center";
				case POSITIVE -> "gui.glowcase.front";
			}));
		...
		{ // We create a button for each alignment possibility of the screen on a 2D canvas (top-left to bottom-right)
			...
			this.addRenderableWidget(
				new StringWidget(
					...
					Component.translatableWithFallback(
						"gui.glowcase.screen.alignment",
						"%s",
						this.blockEntity.macaddress
					),
					this.minecraft.font
				)
			);
			...
			for (int y = 0; y < 3; y++) {
				for (int x = 0; x < 3; x++) {
					Button button = Button.builder(Component.literal(""), action -> {
						...
						for (Button buttonWidget : alignment) {
							...
							if (!buttonWidget.active) {
								blockEntity.xOffset = ScreenBlockEntity.Offset.fromOffset(cur_x - 1);
								blockEntity.yOffset = ScreenBlockEntity.Offset.fromOffset(cur_y - 1);
								break;
							}
						}
					...
					// Current Alignment
					if (blockEntity.xOffset.offset + 1 == x && blockEntity.yOffset.offset + 1 == y) {
						button.active = false;
					}
		...
		this.renderBackfaceWidget = Checkbox.builder(Component.translatable("gui.glowcase.screen.backface"), this.minecraft.font)
			.selected(this.blockEntity.renderBackface)
			.onValueChange((checkbox, checked) -> this.blockEntity.renderBackface = checked)
		...
		this.einkCheckWidget = Checkbox.builder(Component.translatable("gui.glowcase.screen.eink"), this.minecraft.font)
			.selected(this.blockEntity.eink)
			.onValueChange((checkbox, checked) -> this.blockEntity.eink = checked)
		...
		this.stretchCheckWidget = Checkbox.builder(Component.translatable("gui.glowcase.screen.stretch"), this.minecraft.font)
			.selected(this.blockEntity.stretch)
			.onValueChange((checkbox, checked) -> this.blockEntity.stretch = checked)
			.pos(width / 10, height / 2 + 10 + 11)
			.build();
		...
		this.urlEntryWidget.setValue(this.blockEntity.url);
		...
		this.altEntryWidget.setValue(this.blockEntity.alt);
		this.altEntryWidget.setHint(TextUtils.placeholder("gui.glowcase.alt"));
		this.altEntryWidget.setResponder(string -> blockEntity.alt = string);
		...
		if (this.minecraft.options.advancedItemTooltips) {
			this.addRenderableWidget(
				new StringWidget(
					...
					Component.translatableWithFallback(
						"gui.glowcase.screen.mac_address",
						"%s",
						this.blockEntity.macaddress
					).withColor(0x696969),
					this.minecraft.font
	...
	public @Nullable CustomPacketPayload getUpdatePayload() {
		...
		blockEntity.preciseX = (parsedX != null) ? parsedX : 0f;
		blockEntity.preciseY = (parsedY != null) ? parsedY : 0f;
		blockEntity.preciseZ = (parsedZ != null) ? parsedZ : 0f;

		blockEntity.eink = einkCheckWidget.selected();
		blockEntity.stretch = stretchCheckWidget.selected();
		blockEntity.setImage(
			urlEntryWidget.getValue(),
			altEntryWidget.getValue(),
			null
		);

		return C2SEditScreenBlock.of(blockEntity);
	}
}
