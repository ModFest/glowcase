...
import dev.hephaestus.glowcase.mixin.client.bakedbe.sodium.PoseAccessor;
...
public class AsyncEntityRenderer {
	public static final ThreadLocal<AsyncEntityRenderer> INSTANCE = ThreadLocal.withInitial(AsyncEntityRenderer::new);
    private static final int VERTEX_BUFFER_BYTES = NUM_CUBE_FACES * NUM_FACE_VERTICES * EntityVertex.STRIDE;

    private final long[] cubeVertexXY = new long[NUM_CUBE_VERTICES]; // (pos.x, pos.y)
    private final long[] cubeVertexZW = new long[NUM_CUBE_VERTICES]; // (pos.z, color)

    private final int[] cubeFaceNormal = new int[NUM_CUBE_FACES];

	private final Matrix3f prevNormalMatrix = new Matrix3f();

    public void renderCuboid(PoseStack.Pose matrices, VertexBufferWriter writer, ModelCuboid cuboid, int light, int overlay, int color) {
    ...
    private int emitQuads(final long buffer, ModelCuboid cuboid, int overlay, int light) {
        ...
        for (int faceIndex = 0; faceIndex < NUM_CUBE_FACES; faceIndex++) {
            ...
            final int packedNormal = cubeFaceNormal[normals[faceIndex]];
    ...
    private long writeVertex(long ptr, int vertexIndex, long packedUv, long packedOverlayLight, int packedNormal) {
        MemoryUtil.memPutLong(ptr + 0L, cubeVertexXY[vertexIndex]);
        MemoryUtil.memPutLong(ptr + 8L, cubeVertexZW[vertexIndex]); // overlaps with color attribute
    ...
    private void prepareVertices(PoseStack.Pose matrices, ModelCuboid cuboid, int color) {
    ...
    private void setVertex(int vertexIndex, float x, float y, float z, int color) {
        ...
        cubeVertexXY[vertexIndex] = Int2.pack(Float.floatToRawIntBits(x), Float.floatToRawIntBits(y));
        cubeVertexZW[vertexIndex] = Int2.pack(Float.floatToRawIntBits(z), color);
    }

    private void prepareNormalsIfChanged(PoseStack.Pose matrices) {
        if (matrices.normal().equals(prevNormalMatrix)) {
            return;
        }

		boolean trustedNormals = ((PoseAccessor) (Object) matrices).getTrustedNormals();
		cubeFaceNormal[FACE_NEG_Y] = MatrixHelper.transformNormal(matrices.normal(), trustedNormals, Direction.DOWN);
        cubeFaceNormal[FACE_POS_Y] = MatrixHelper.transformNormal(matrices.normal(), trustedNormals, Direction.UP);
        cubeFaceNormal[FACE_NEG_Z] = MatrixHelper.transformNormal(matrices.normal(), trustedNormals, Direction.NORTH);
        cubeFaceNormal[FACE_POS_Z] = MatrixHelper.transformNormal(matrices.normal(), trustedNormals, Direction.SOUTH);
        cubeFaceNormal[FACE_POS_X] = MatrixHelper.transformNormal(matrices.normal(), trustedNormals, Direction.WEST);
        cubeFaceNormal[FACE_NEG_X] = MatrixHelper.transformNormal(matrices.normal(), trustedNormals, Direction.EAST);
